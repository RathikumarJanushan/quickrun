...
class AvailableUser extends StatelessWidget {
  const AvailableUser({Key? key}) : super(key: key);
  ...
  Widget _buildBody(BuildContext context, List<DocumentSnapshot> documents) {
    ...
    List<Widget> busyUsers = []; // New list for busy users
    ...
    for (var doc in documents) {
        ...
        color: status == 'start'
            ...
            : status == 'end'
                ...
                : status == 'break'
                    ...
                    : Colors.blue.withOpacity(0.8), // New color for "busy"
        child: ListTile(
          ...
          subtitle: Column(
            ...
            children: [
              ...
              SizedBox(height: 4.0),
              Text(
                'Status: ${status == 'start' ? 'Available' : status == 'end' ? 'Unavailable' : status == 'break' ? 'Brake' : 'Busy'}',
                style: TextStyle(
                  fontSize: 14.0,
                  fontWeight: FontWeight.bold,
                  color: Colors.white,
      ...
      if (status == 'start') {
        ...
        busyUsers.add(userCard); // Add to busy list
      }
    }
    ...
    return ListView(
      children: [
        ...
        if (unavailableUsers.isNotEmpty ||
            brakeUsers.isNotEmpty ||
            busyUsers.isNotEmpty)
          Column(
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 20.0,
                      fontWeight: FontWeight.bold,
                      color: const Color.fromARGB(255, 236, 221, 6),
                    ),
                  ),
                ),
              SizedBox(height: 8.0),
              ...busyUsers, // Add busy users to the UI
            ],
          ),
      ],
    );
